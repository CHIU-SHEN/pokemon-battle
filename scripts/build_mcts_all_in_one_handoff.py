#!/usr/bin/env python3
"""Build the unified CPU-collection and V100-training server handoff."""
...
PACKAGE = "mcts-teacher-v2-all-in-one"
...
FILES = (
    "config/top2_rl_policy.json", "requirements-train.txt", "requirements-eval.txt",
    "scripts/collect_top2_mcts.py", "scripts/audit_mcts_collection.py",
    "scripts/build_mcts_primary_dataset.py", "scripts/verify_mcts_primary_dataset.py",
    "scripts/train_top2_mcts.py", "scripts/run_mcts_teacher_smoke.py",
    "scripts/verify_mcts_all_in_one_handoff.py", "jobs/mcts_teacher_v2_all_in_one.sh",
    "data/cards.json", "data/card_tags.json",
    "data/high_score_decks/crustle_kangaskhan_cage/deck.csv",
)
FROZEN = ("artifacts/sl0_shared_full/best.pt", "artifacts/adapters_top10/crustle_kangaskhan_cage/best.pt", AUTHORITATIVE, AUTHORITATIVE + ".sha256")
...
def build(output: Path, *, code_root: Path, frozen_root: Path):
    ...
    members = []
    for relative in (*DIRECTORIES, *FILES): members.extend(_copy(code_root, package_root, relative))
    missing = []
    for relative in FROZEN:
        copied = _copy(frozen_root, package_root, relative)
        members.extend(copied)
        if not copied: missing.append(relative)
    ...
    manifest = {"schema_version": "mcts_teacher_v2_all_in_one_v1", "branch": "primary", "target_games": 10000, "default_workers": 16, "authoritative_archive_sha256": AUTHORITATIVE_SHA256, "missing_frozen_files": missing, "files": {}}
    for relative in sorted(set(members)): manifest["files"][relative] = sha256_file(package_root / relative)
    (package_root / "HANDOFF_MANIFEST.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    archive = output / f"{PACKAGE}.tar.gz"
    with tarfile.open(archive, "w:gz") as bundle: bundle.add(package_root, arcname=PACKAGE)
...
def main() -> int:
    ...
    archive, checksum, manifest = build(args.output_dir.resolve(), code_root=root, frozen_root=args.frozen_root.resolve())
